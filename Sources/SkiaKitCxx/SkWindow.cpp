...
bool Window::onChar(SkUnichar c, SC::ModifierKey modifiers) {
    return this->signalLayers([=](Layer* layer) { return layer->onChar(c, modifiers); });
}

bool Window::onKey(SC::Key key, SC::InputState state, SC::ModifierKey modifiers) {
    return this->signalLayers([=](Layer* layer) { return layer->onKey(key, state, modifiers); });
}

bool Window::onMouse(int x, int y, SC::InputState state, SC::ModifierKey modifiers) {
    return this->signalLayers([=](Layer* layer) { return layer->onMouse(x, y, state, modifiers); });
}

bool Window::onMouseWheel(int x, int y, float deltax, float deltay, SC::ModifierKey modifiers) {
    return this->signalLayers([=](Layer* layer) { return layer->onMouseWheel(x, y, deltax, deltay, modifiers); });
}

bool Window::onTouch(intptr_t owner, SC::InputState state, float x, float y) {
    return this->signalLayers([=](Layer* layer) { return layer->onTouch(owner, state, x, y); });
}

bool Window::onFling(SC::InputState state) {
    return this->signalLayers([=](Layer* layer) { return layer->onFling(state); });
}

bool Window::onPinch(SC::InputState state, float scale, float x, float y) {
...
void Window::onClose(){
    ...
    this->visitLayers([=](Layer* layer) { layer->onClose(); });
}
